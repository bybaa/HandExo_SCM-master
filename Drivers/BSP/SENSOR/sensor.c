...
void read_change(void)
{
  	...
  	uint8_t Ocf[6],Cof[6],Lin[6];							//OCF为偏差补偿完成标识,COF为CARDIC单元溢出标识，LIN为磁场线性度报警
...
/// ---------------------------------------------------------	
	...
	for(xun=0;xun<6;xun++)
	{
		...
		}									//补偿算法完成OCF=1，没有溢出COF=0和线性度报警LIN=0
  		else
		{	
			out[xun]=4096;
		}															//AS5045范围0-4095，出现4096则说明磁铁安装不合适，需要调整。
